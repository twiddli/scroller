...
const members = {
  ...
  */
  ...
  scrollBy(left, top, animate) {
    ...
    const startLeft = self.__isAnimating ? self.__scheduledLeft : self.__scrollLeft;
    const startTop = self.__isAnimating ? self.__scheduledTop : self.__scrollTop;
  ...
  */
  ...
  __startDeceleration() {
    ...
    if (self.options.paging) {
      const scrollLeft = Math.max(Math.min(self.__scrollLeft, self.__maxScrollLeft), 0);
      const scrollTop = Math.max(Math.min(self.__scrollTop, self.__maxScrollTop), 0);
    ...
    const minVelocityToKeepDecelerating = self.options.snapping ? 4 : 0.001;
  ...
  __stepThroughDeceleration(render) {
    ...
    if (self.options.bouncing) {
      ...
      const { penetrationDeceleration, penetrationAcceleration } = self.options;
